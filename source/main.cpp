...
double angleForChunk(const int const current, const int total)
{
	...
	return angles[current % (total/2)];
}

template<typename T>
Eigen::Matrix3d hat(const T& v)
{
	Eigen::Matrix3d vhat;
	vhat <<
		0, -v.z, v.y,
...
public:
	...
	void saveCapturedFrames(std::string dataFolder)
	{
		...
		for (auto &colorImage : mColorImages)
		{
			...
			// write to file
			std::string filename = (dataFolder + "/raw_color_");
			filename += to_string_extended(imageIndex + 1);
			filename += ".png";
			try {
				cv::imwrite(filename.c_str(), cvColorImage);
				LOG_S(INFO) << "Successfully saved raw color data to " << filename;
			}
			catch (std::runtime_error& ex) {
				LOG_S(ERROR) << "Exception converting " << filename << " image to BMP format: " << ex.what();
				continue;
			}
		...
		for (auto &depthImage : mDepthImages)
		{
			...
			// write to file
			std::string filename = (dataFolder + "/raw_depth_");
			filename += to_string_extended(imageIndex + 1);
			filename += ".png";
			try {
				cv::imwrite(filename.c_str(), cvDepthImage);
				LOG_S(INFO) << "Successfully saved raw depth data to " << filename;
			}
			catch (std::runtime_error& ex) {
				LOG_S(ERROR) << "Exception converting " << filename << " image to BMP format: " << ex.what();
				continue;
		...
		for (auto &bodyMask : mIndexImages)
		{
			...
			// write to file
			std::string filename = (dataFolder + "/raw_body_mask_");
			filename += to_string_extended(imageIndex + 1);
			filename += ".png";
			try {
				cv::imwrite(filename.c_str(), cvBodyMask);
				LOG_S(INFO) << "Successfully saved body mask data to " << filename;
			}
			catch (std::runtime_error& ex) {
				LOG_S(ERROR) << "Exception converting " << filename << " image to BMP format: " << ex.what();
				continue;
	...
	void constructPointClouds(KinectDataProvider* KDP)
	{
		...
		std::unique_ptr<PointPair[]> bboxHighRes = std::unique_ptr<PointPair[]>(new PointPair[mNumChunks], std::default_delete<PointPair[]>());
		std::unique_ptr<PointPair[]> bboxLowRes = std::unique_ptr<PointPair[]>(new PointPair[mNumChunks], std::default_delete<PointPair[]>());
		...
		for (int curChunkIdx = 0; curChunkIdx < mNumChunks; curChunkIdx++)
		{
			...
			for (int i = 0;i < mChunkSize;i++)
			{
				...
				{
					std::string filename = "data/cropped_color_";
					filename += to_string_extended(seriesImageIdx + 1);
					filename += ".png";
					try {
						cv::imwrite(filename.c_str(), cvColorImages[i]);
						LOG_S(INFO) << "Successfully saved cropped color data to " << filename;
					}
					catch (std::runtime_error& ex) {
						LOG_S(ERROR) << "Exception converting " << filename << " image to png format: " << ex.what();
						continue;
				...
				{
					std::string filename = "data/cropped_depth_";
					filename += to_string_extended(seriesImageIdx + 1);
					filename += ".png";
					try {
						cv::imwrite(filename.c_str(), cvDepthImages[i]);
						LOG_S(INFO) << "Successfully saved cropped depth data to " << filename;
					}
					catch (std::runtime_error& ex) {
						LOG_S(ERROR) << "Exception converting " << filename << " image to png format: " << ex.what();
						continue;
		...
		*/
		...
		// generate point clouds
		{
			...
			for (int curChunkIdx = 0; curChunkIdx < mNumChunks; curChunkIdx++)
			{
				...
				for(auto& p : (*mPointClouds[curChunkIdx]))
				{
					//p.z /= 1000.;
					//kinect2_undistort(p.z*(p.x / 3.75 - 257.226) / 365.155, p.z*(p.y / 2.54716981132 - 206.719) / 365.155, p.x, p.y);

					DepthSpacePoint dsp;
					dsp.X = p.x / 3.75;
					dsp.Y = p.y / 2.54716981132;
	...
	void showPointClouds()
	{
		...
		for (int curChunkIdx = 0; curChunkIdx < mPointClouds.size(); curChunkIdx++)
		{
	...
	void createBenchmarkProblem(const int size = 2, const double stepSize = 0.05)
	{
		...
		for (double x = 0; x <= 1; x+= stepSize)
		{
			for (double y = 0;y <= 1; y+= stepSize)
			{
				...
				p.g = 0;
				p.b = 0.;
				mPointClouds[0]->push_back(p);
			}
		}
		auto rotaxis = Eigen::Vector3d(0, 1, 0);
	...
	void reconstructAvatar()
	{
		...
		for (int f = 0; f < mPointClouds.size(); f++)
		{
			...
			// 0.0175 = degree to radians
			//Eigen::Affine3f trafo3; trafo3 = Eigen::AngleAxisf(0.0175 * 13., Eigen::Vector3f::UnitX());
			//pcl::transformPointCloud(*mPointClouds[f], *mPointClouds[f], trafo3);

			Eigen::AngleAxisf aa(-0.0175*(angleForChunk(f, mPointClouds.size()) + backsideRotation), Eigen::Vector3f::UnitY());
		...
		
		{
			...
			{
				pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGBL> rgb(labeledPointClouds[curChunkIdx]);
				viewer->addPointCloud<pcl::PointXYZRGBL>(labeledPointClouds[curChunkIdx], rgb, "sample cloud" + std::to_string(curChunkIdx));
				viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "sample cloud" + std::to_string(curChunkIdx));
			}
			viewer->initCameraParameters();
							...
							for (int n = 0; n < labeledPointClouds[f]->size(); n++)
							{
								...
								for (int i = 0; i < numFoundPoints; i++)
								{
									...
									*/
									...
									tripletIndex++;
									triplets[tripletIndex] = { rowf + 1, colf + 3 + 0 , -pOverVar*yfnhat(1,0) };
									tripletIndex++;
									triplets[tripletIndex] = { rowf + 1, colf + 3 + 2 , -pOverVar*yfnhat(1,2) };
									tripletIndex++;
									triplets[tripletIndex] = { rowf + 2, colf + 3 + 0 , -pOverVar*yfnhat(2,0) };
									tripletIndex++;
									...
									*/
									...
									tripletIndex++;
									triplets[tripletIndex] = { rowf + 1, colg + 3 + 0 , pOverVar*ygmhat(1,0) };
									tripletIndex++;
									triplets[tripletIndex] = { rowf + 1, colg + 3 + 2 , pOverVar*ygmhat(1,2) };
									tripletIndex++;
									triplets[tripletIndex] = { rowf + 2, colg + 3 + 0 , pOverVar*ygmhat(2,0) };
									tripletIndex++;
									...
									*/
									...
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colf + 0 + 0 , pOverVar*yfnhat(1,0) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colf + 0 + 2 , pOverVar*yfnhat(1,2) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 2, colf + 0 + 0 , pOverVar*yfnhat(2,0) };
									tripletIndex++;
									...
									*/
									...
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 0, colf + 3 + 2 , -pOverVar*yfnhat2(0,2) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colf + 3 + 0 , -pOverVar*yfnhat2(1,0) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colf + 3 + 1 , -pOverVar*yfnhat2(1,1) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colf + 3 + 2 , -pOverVar*yfnhat2(1,2) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 2, colf + 3 + 0 , -pOverVar*yfnhat2(2,0) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 2, colf + 3 + 1 , -pOverVar*yfnhat2(2,1) };
									tripletIndex++;
									...
									*/
									...
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colg + 0 + 0 , -pOverVar*yfnhat(1,0) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colg + 0 + 2 , -pOverVar*yfnhat(1,2) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 2, colg + 0 + 0 , -pOverVar*yfnhat(2,0) };
									tripletIndex++;
									...
									*/
									...
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 0, colg + 3 + 2 , pOverVar*yfngmhat(0,2) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colg + 3 + 0 , pOverVar*yfngmhat(1,0) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colg + 3 + 1 , pOverVar*yfngmhat(1,1) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 1, colg + 3 + 2 , pOverVar*yfngmhat(1,2) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 2, colg + 3 + 0 , pOverVar*yfngmhat(2,0) };
									tripletIndex++;
									triplets[tripletIndex] = { rowg + 2, colg + 3 + 1 , pOverVar*yfngmhat(2,1) };
									tripletIndex++;
...
#pragma omp parallel for
...

		{
			...
			for (int f = 0; f < labeledPointClouds.size(); f++)
			{
				...
				std::string filename = "data/point_cloud_rigid_registered" + to_string_extended(f);
			...
			std::string filename = "data/point_cloud_rigid_merged";
			LOG_S(INFO) << "Saving back results...";
			try {
		...
		for (int f = 0; f < labeledPointClouds.size(); f++)
		{
			...
			completePointCloud += *labeledPointClouds[f];
			pcl::copyPointCloud(*labeledPointClouds[f], *mPointClouds[f]);
			std::string filename = "data/point_cloud_nonrigid_registered" + to_string_extended(f);
			LOG_S(INFO) << "Saving back results...";
			try {
		...
		std::string filename = "data/point_cloud_nonrigid_merged";
		LOG_S(INFO) << "Saving back results...";
		try {
...

		{
			...
			do
			{
				...
				for (int f = 0; f < labeledPointClouds.size(); f++)
				{
					...
					for (auto const &ent1 : jointEstimations[f])
					{
						for (auto const &ent2 : ent1.second)
						{
							...
							jointCentroid.get(yfab);
							auto yfabhat = hat(yfab);
							A.block<3, 3>(row, cola) += 2 * alpha*Eigen::Matrix3d::Identity();
							// -y_f,ab hat
							A.block<3, 3>(row, cola + 3) += -2 * alpha*yfabhat;
							// -I3
							A.block<3, 3>(row, colb) += -2 * alpha*Eigen::Matrix3d::Identity();
							// y_f,ab hat
							A.block<3, 3>(row, colb + 3) += 2 * alpha*yfabhat;
							// next row
							// y_f,ab hat
							A.block<3, 3>(row+3, cola) += 2 * alpha*yfabhat;
							// -y_f,ab hat * y_f,ab hat
							A.block<3, 3>(row+3, cola + 3) += -2 * alpha*yfabhat*yfabhat;
							// -y_f,ab hat
							A.block<3, 3>(row+3, colb ) += -2 * alpha*yfabhat;
							// y_f,ab hat * y_f,ab hat
							A.block<3, 3>(row+3, colb + 3) += 2 * alpha*yfabhat*yfabhat;
				...
				*/
				...
				iteration++;
				LOG_F(INFO, "%-5i | %-15f | %-15i | %-15i | %-15i | %-15i | %10f", iteration, error, KDTreeRebuildTime, VarianceRebuildTime, EQBuildTime, EQSolveTime);
